...
//     try {
//       const response = await fetch(`http://localhost:9090/api/students/${studentId}/modules`);
//       const student = await response.json();
//       const moduleNames = student.modules.map(module => `<li>${module.name} - ${module.credits} Credits, Level ${module.level}</li>`);

//       Swal.fire({
//         title: "Enrolled Modules",
//         html: `<ul>${moduleNames.join("")}</ul>`,
//         icon: "info",
...
const TableComponent = () => {
  ...
  const showModules = async (studentId) => {
    try {
      const response = await fetch(`http://localhost:9090/api/students/${studentId}/modules`);
      const student = await response.json();
      const moduleNames = student.modules.map(
        (module) => `<li>${module.name} - ${module.credits} Credits, Level ${module.level}</li>`
      );
  ...
  const downloadPDF = () => {
    ...
    doc.autoTable({
      head: [["#", "Student Name", "Tel. No", "Email", "Birth Day", "Department", "Current Level"]],
      body: paginatedData.map((student, index) => [
        index + 1,
    ...
    <div className="p-6 h-full w-full bg-customColor rounded-lg shadow-lg">
      ...
      <div className="p-4 w-full bg-white rounded-lg shadow-lg">
        ...
        <div className="flex justify-between mt-4">
          <div>
            <button onClick={downloadPDF} className="bg-green-500 text-white px-3 py-1 rounded">
              Download PDF
            </button>
          </div>
          <div>
